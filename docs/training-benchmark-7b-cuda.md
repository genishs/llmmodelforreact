...
## 8060S seq512 재학습본 추가 비교 (2026-06-17)

AMD 8060S 팀이 위 결과를 받아 **seq 384 → 512로 올려 재학습**(fp16, 48GB 카브아웃, 4060과
동일한 303개 데이터, val_loss 4.38, 43.9분, OOM 0회)하고 어댑터를 push했다
(`models/qwen-react-lora-7b-seq512`). **같은 4060 · 같은 4비트 베이스 · greedy**로 직접
head-to-head 재검증한 결과:

| 테스트 | AMD-v4 (seq384) | **AMD-8060S (seq512)** | CUDA-QLoRA (seq768) |
|--------|-----------------|------------------------|---------------------|
| 카운터(EN) | 정지 실패(오버런) | 동작하나 `THOOK` 특수토큰 누수·오버런 | 깔끔히 정지 |
| useWindowSize(KR) | 양호(장황) | 양호하나 뒤에 `### kInstruction` 누수 | 양호(간결) |
| useDebounce(TS) | **깨짐** | **해소** — 단, 제네릭 아님(`value: string`) + `###_AdjustorThunk` 누수 | **완벽** — `useDebounce<T>` 제네릭 |
| TanStack 무한스크롤 | **실패** | **해소** — 전체 구현 | 성공 |
| **EgovPaging.jsx (840tok)** | 빈 출력 | **여전히 빈 출력** | **정상 변환** |

**해석:**
- seq 384 → 512로 올린 것만으로 **TS 제네릭·복잡 프롬프트는 해소**됐다(AMD 재학습의 성과).
  데이터 잘림(seq 길이)이 그 실패들의 직접 원인이었음을 양쪽에서 교차 확인.
- 남은 격차 둘: ① seq512는 **특수토큰/후속 지시문 누수**가 잦고(정지 품질↓), 제네릭 추론이
  약함. ② **긴 실파일(840tok)은 여전히 붕괴** — 학습 분포 길이의 문제다. seq512(학습 최대
  ~469)는 840과 간격이 너무 크고, seq768(학습 최대 713)은 840까지 일반화된다.
- 즉 **일반/중간 길이는 8060S seq512로 4060급 도달**, **긴 실파일은 CUDA seq768만 처리**.
  DirectML은 VRAM 천장 탓에 seq를 768로 더 못 올리는 게 잔여 한계(→ CUDA 4비트의 이점).
