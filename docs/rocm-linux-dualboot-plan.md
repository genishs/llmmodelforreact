...
### 0단계: 디스크 준비 (Windows에서, 사용자가 관리자 권한으로 직접) — ✅ 2026-07-12 완료
> ✅ **완료**: **미할당 230.5GB 확보 성공** (D: 931.5GB → 701GB). 범인은 볼륨 끝에 박힌 `$Mft::$BITMAP`이라
> Windows 기본 축소는 40GB 한계 → **파티션 매니저(PreOS 모드 MFT 재배치)로 해결.** 상세·교훈은 `docs/disk-prep-runbook.md` 정본.
> ⚠️ 아래는 히스토리(축소 전 현실). 예전 "D: 새 볼륨/여유 187GB" 서술은 낡음이라 정정됨.
