...
| **141B 킹왕짱** (Mixtral-8x22B) | HQQ **2bit** | ROCm | ✅ **완주(seq한계)** | val **0.7132**, **seq1280(기존 2.5배)** 2에폭 78스텝, 10.5h(471s/step), GTT피크 **49.35GiB**/56캡(seq1280도 여유), 어댑터 139.5MB. 2026-08-02 완주. **in-process seq프로브로 최대seq 판정**(`probe_141b_maxseq.py`) — grad-ckpt로 메모리가 seq 거의 안 먹어 병목=시간. **판정: seq 2.5배 자체는 val 개선 없음**(ReAct 샘플 짧아 긴 컨텍스트=패딩), 원본(0.62,3ep) 못 넘음=에폭 교란(2vs3). **성과=141B seq1280 안정학습 실증** |
...
> **✅ 2026-08-02 재실행 완료**: 123B-v2 재학습(val **0.4642**, 챔피언 0.4704 재현) · 72B 4bit 재학습(val **0.5706**) ·
> **141B 킹왕짱 seq1280×2ep 완주(val 0.7132)** — 전 모델 학습 완주.
> **heldout 채점은 두목 참관용으로 보류**: llama.cpp Vulkan 경로 구축·하니스 버그수정 완료(600초 하드타임아웃→동적 `max(1800,max_new*3+600)`, 태스크별 실패격리로 부분결과 보존).
> 72B는 생성 7/7 완료(score_v2만 남음), 123B/141B는 base→GGUF **Q2_K** 변환 각 ~2h 필요(Q4는 RAM 60G 초과라 적재불가). §3 점수표는 채점 후 기재.
