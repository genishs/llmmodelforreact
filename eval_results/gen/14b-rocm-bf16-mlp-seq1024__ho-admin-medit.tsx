...
function EgovAdminMemberEdit({ mode }: Props) {
  ...
  const [groupCodeOptions, setGroupCodeOptions] = useState<Option[]>([]);
  ...
  const formObjValidator = (checkRef: React.MutableRefObject<HTMLInputElement[]>) => {
    if (checkRef.current[0].value === "") {
      alert("시용자 ID는 필수 값입니다.");
      return false;
    }
          ...
          <div className="join_box d_flex aic">
            ...
            <select id="deptCd" name="deptCd" title="소속 부서 선택" className="join_ser4"
              onChange={async (e) => {
                ...
                setMemberDetail({
                  ...memberDetail, deptCd: selectedDept
                });

                try {
                  const resp = await axios.get("/geoportal/api/api/deptSubList", {
                    params: { deptCd: e.target.value },
                    withCredentials: true,
                  });

                  let deptSubCdOptions: Option[] = [];
                  (resp.data.deptSubList_result || []).forEach(item => {
                    deptSubCdOptions.push({ value: item.code, label: item.codeNm });
                  });
