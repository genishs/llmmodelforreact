  ...
  const retrieveList = useCallback(async (srchCnd: SearchCondition): Promise<void> => {
    ...
    try {
      ...
      const mutListTag: JSX.Element[] = resultList.map((item, index) => {
        ...
        return <tr key={listIdx} onClick={() => handleRowClick(item)}><td>{listIdx}</td><td>{item.mberId}</td><td>{item.mberNm}</td><td>{authNm}</td><td>{item.sbscrbDe}</td><td>{["P", "A"].includes(item.mberSttus) ? "가능" : "탈퇴"}</td></tr>;
      });
      if (!mutListTag.length) mutListTag.push(<tr key="0"><td colSpan={6}>검색된 결과가 없습니다.</td></tr>);
