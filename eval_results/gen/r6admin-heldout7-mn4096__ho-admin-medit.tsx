...
function EgovAdminMemberEdit({ mode }: Props): JSX.Element {
  ...
  const retrieveDetail = (): void => {
    ...
    const fetchSubDept = async (deptCd: string, currentSubCd?: string): Promise<void> => {
      try {
        ...
        if (currentSubCd && subOptions.some((opt) => opt.value === currentSubCd)) setMemberDetail((prev) => ({ ...prev, deptSubCd: currentSubCd }));
        else setMemberDetail((prev) => ({ ...prev, deptSubCd: "" }));
    ...
    EgovNet.requestFetch(retrieveDetailURL, requestOptions, (resp) => {
      ...
      groupCodeOptions = [];
      resp.result.groupId_result.forEach((item: { code: string; codeNm: string }) => groupCodeOptions.push({ value: item.code, label: item.codeNm }));
      setGroupCodeOptions(groupCodeOptions);
      setDeptCdOptions(resp.result.deptList_result.map((item: { code: string; codeNm: string }) => ({ value: item.code, label: item.codeNm })));
      setdeptSubCdOptions(resp.result.deptSubList_result.map((item: { code: string; codeNm: string }) => ({ value: item.code, label: item.codeNm })));
    });
  };
  ...
  const checkIdDplct = (): Promise<number> => {
    return new Promise((resolve) => {
      ...
      EgovNet.requestFetch(checkIdURL, reqOptions, (resp) => {
        if (resp.resultCode === CODE.RCV_SUCCESS && resp.result.usedCnt > 0) { setMemberDetail((prev) => ({ ...prev, checkIdResult: "이미 사용중인 아이디입니다." })); resolve(resp.result.usedCnt); setIdCheckStatus({ checked: true, available: false }); }
        else { setMemberDetail((prev) => ({ ...prev, checkIdResult: "사용 가능한 아이디입니다." })); resolve(0); setIdCheckStatus({ checked: true, available: true }); }
      });
    });
  };
  ...
  const updateMember = (): void => {
    ...
    if (modeStr === "POST") {
      ...
      Object.keys(memberDetail).forEach((key) => formData.append(key, memberDetail[key]));
