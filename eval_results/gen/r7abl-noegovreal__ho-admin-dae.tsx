...
function EgovAdminDataAccessEdit({ mode }: { mode: string }) {
  ...
  const retrieveDetail = () => {
    if (modeInfo.mode === CODE.MODE_MODIFY) {
      const uniqId = location.state?.uniqId as string;
      ...
      EgovNet.requestFetch<Detail>(retrieveDetailURL, requestOptions, (resp) => {
  ...
  const updateBoard = () => {
    ...
    EgovNet.requestFetch<{ resultCode: number; resultMessage: string }>(
      modeInfo.editURL,
      requestOptions,
      (resp) => {
      ...
      <div className="c_wrap">
        ...
        <div className="layout">
          <div className="contents BOARD_CREATE_REG" id="contents">
            ...
            {modeInfo.mode === CODE.MODE_CREATE && <h2 className="tit_2">데이터 접근권한 등록</h2>}
            {modeInfo.mode === CODE.MODE_MODIFY && <h2 className="tit_2">데이터 접근권한 수정</h2>}
            <div className="board_view2">
              <dl><dt>권한 ID</dt><dd>{detail.id}</dd></dl>
              <dl><dt>리소스명</dt><dd>{detail.resourceName}</dd></dl>
              {/* Add more fields as needed based on actual data structure */}
            </div>
            <div className="board_btn_area">
              <div className="left_col btn1">
                <button className="btn btn_skyblue_h46 w_100" onClick={updateBoard}>저장</button>
                {modeInfo.mode === CODE.MODE_MODIFY && (
                  <button className="btn btn_skyblue_h46 w_100" onClick={() => deleteBoard(detail.id!)}>삭제</button>
                )}
              </div>
              <div className="right_col btn1"><Link to={URL.ADMIN_DATAACCESS} className="btn btn_blue_h46 w_100">목록</Link></div>
            </div>
          </div>
        </div>
      </div>
