...
const EgovAttachFile: React.FC<EgovAttachFileProps> = ({
  boardFiles,
  mode,
  fnChangeFile,
  fnDeleteFile,
  ...
  const onClickDownFile = (atchFileId: string, fileSn: number) => {
    window.open(
      SERVER_URL + "/file?atchFileId=" + atchFileId + "&fileSn=" + fileSn + ""
    );
  };

  const onClickDeleteFile = (atchFileId: string, fileSn: number, fileIndex: number) => {
