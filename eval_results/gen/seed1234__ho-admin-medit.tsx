...
function EgovAdminMemberEdit({ mode }: { mode: number }) {
  const navigate = useNavigate();
  const location = useLocation();
  const checkRef = useRef<{ [key: string]: HTMLInputElement | null }>({});
  ...
  const retrieveDetail = () => {
    ...
    const fetchSubDept = async (deptCd: string, currentSubCd?: string) => {
      try {
        ...
        if (currentSubCd && subOptions.some((opt: any) => opt.value === currentSubCd)) {
          setMemberDetail((prev: MemberDetail) => ({ ...prev, deptSubCd: currentSubCd }));
        } else {
          setMemberDetail((prev: MemberDetail) => ({ ...prev, deptSubCd: "" }));
        }
      } catch (error) {
    ...
    EgovNet.requestFetch(retrieveDetailURL, requestOptions, (resp: any) => {
      if (modeInfo.mode === CODE.MODE_MODIFY) {
        ...
        if (resp.result.mberManageVO.deptCd) {
          fetchSubDept(resp.result.mberManageVO.deptCd, resp.result.mberManageVO.deptSubCd);
        }
      }
      groupCodeOptions = [];
      resp.result.groupId_result.forEach((item: any) => {
        groupCodeOptions.push({ value: item.code, label: item.codeNm });
      });
      setGroupCodeOptions(groupCodeOptions);
      let deptOptions = [];
      resp.result.deptList_result.forEach((item: any) => {
      ...
      resp.result.deptSubList_result.forEach((item: any) => {
  ...
  const checkIdDplct = () => {
    return new Promise<number>((resolve) => {
      ...
      EgovNet.requestFetch(checkIdURL, reqOptions, (resp: any) => {
        if (resp.result.usedCnt > 0) {
          setMemberDetail({ ...memberDetail, checkIdResult: "이미 사용중인 아이디입니다." });
  ...
  useEffect(() => {
    if (memberDetail.deptCd) {
      const fetchSubDept = async () => {
        ...
        const deptSubCdOptions = (resp.data.deptSubList_result || []).map((item: any) => ({ value: item.code, label: item.codeNm }));
        setdeptSubCdOptions(deptSubCdOptions);
        if (memberDetail.deptSubCd && !deptSubCdOptions.some((opt: any) => opt.value === memberDetail.deptSubCd)) {
          setMemberDetail((prev: MemberDetail) => ({ ...prev, deptSubCd: "" }));
        }
      };
      fetchSubDept();
  ...
  return (
    <div className="new_wrap2" id="warp2">
      <div className="w100 d_flex d_jcc"><div className="w1200">
        <div className="nav_wrap d_flex d_end">
          <div className="nav d_flex d_end">
            ...
            <div className="nav_ico_arr img_all"></div>
            <Link><div className="nav_txt">{modeInfo.mode === CODE.MODE_CREATE ? '사용자 등록' : '사용자 수정'}</div></Link>
          </div>
        </div>
      </div></div>
      <div className="con_wrap d_flex d_jcc">
        <div className="join">
          ...
          <div className="join_box d_flex aic">
            ...
            <select id="groupId" name="groupId" title="회원권한유형선택" className="join_ser3" onChange={(e) => setMemberDetail({ ...memberDetail, groupId: e.target.value })} value={memberDetail.groupId}>{groupCodeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
          </div>
          <div className="join_box d_flex aic">
            ...
            <select id="deptCd" name="deptCd" title="소속 부서 선택" className="join_ser4" onChange={async (e) => {
              ...
              try {
                ...
                let deptSubCdOptions = [];
                (resp.data.deptSubList_result || []).forEach((item: any) => { deptSubCdOptions.push({ value: item.code, label: item.codeNm }); });
                setdeptSubCdOptions(deptSubCdOptions);
              } catch (error) { console.error("하위 부서 목록 불러오기 실패", error); }
            }} value={memberDetail.deptCd}>{deptCdOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
            <div className="mr20"></div>
            {deptSubCdOptions.length > 0 && <select className="join_ser4" id="deptSubCd" name="deptSubCd" value={memberDetail.deptSubCd} onChange={(e) => setMemberDetail({ ...memberDetail, deptSubCd: e.target.value })}>{deptSubCdOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>}
          </div>
          <div className="grey_line"></div>
          <div className="pop_btn_wrap"><div className="blu_btn" onClick={updateMember}>저 장</div>{modeInfo.mode === CODE.MODE_MODIFY && <div className="blu_btn" onClick={() => deleteMember(memberDetail.uniqId)}>삭 제</div>}<Link to={URL.ADMIN_MEMBERS}><div className="w_btn">목 록</div></Link></div>
        </div>
      </div>
      <div className="c_wrap"><div className="location"></div></div>
    </div>
  );
}
