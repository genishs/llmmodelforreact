...
function EgovPaging({ pagination, moveToPage }: Props) {
  ...
  const paginationTag = Array.from({ length: currentLastPage - currentFirstPage + 1 }, (_, i) =>
    i + currentFirstPage === currentPageNo ?
      <div key={i + currentFirstPage}><button className="num_on"><div className="num_on1" />{i + currentFirstPage}</button></div> :
      <div key={i + currentFirstPage}><button className="num_off" onClick={() => moveToPage(i + currentFirstPage)}>{i + currentFirstPage}</button></div>
  );

  return (
    <div className="num_wrap d_flex d_jcc aic">
      <button className="num_l img_all" onClick={() => currentPageNo > 1 && moveToPage(currentPageNo - 1)} style={{ opacity: currentPageNo > 1 ? 1 : 0.5 }} tabIndex="0" />
      {paginationTag}
      <button className="num_r img_all" onClick={() => currentPageNo < totalPageCount && moveToPage(currentPageNo + 1)} style={{ opacity: currentPageNo < totalPageCount ? 1 : 0.5 }} tabIndex="0" />
    </div>
  );
}
