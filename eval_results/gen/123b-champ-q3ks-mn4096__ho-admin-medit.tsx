...
function EgovAdminMemberEdit(props: EgovAdminMemberEditProps) {
  ...
  const currentPath = location.pathnam;
  ...
  const [memberDetail, setMemberDetail] = useState<Record<string, any>>({});
  ...
  const initMode = () => {
    switch (props.mode) {
      ...
      case CODE.MODE.MODIFY:
        ...
        break;
      default:
        navigate({ pathnam: URL.ERROR }, { state: { msg: "" } });
    }
    retrieveDetail();
  };
  ...
  const retrieveDetail = () => {
    ...
    if (modeInfo.mode === CODE.MODE.CREATE) {
      setMemberDetail({
        tm